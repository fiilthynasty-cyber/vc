...
import BackendTest from './components/BackendTest'; // FIXED import

type Tab = 'builder' | 'casino' | 'system';

const TAB_LABELS: Record<Tab, { title: string; desc: string }> = {
  builder: { title: 'PROMPT BUILDER', desc: 'Construct & AI-refine your prompts using modular components' },
  casino: { title: 'CASINO SPINNER', desc: 'Spin the reels for randomly generated prompt combinations' },
  system: { title: 'SYSTEM MONITOR', desc: 'Backend health, service map & live console output' },
};
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <div key={i} className="bg-black bg-opacity-40 rounded p-2 border border-gray-900">
                  <p className="font-mono-tech text-xs break-all" style={{ color: f.color }}>{f.fn}</p>
                </div>
              ))}
            </div>
          </div>
